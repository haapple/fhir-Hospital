...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // 允許沒有 origin 的請求（直接 IP 訪問、Postman 等）
    if (!origin) {
      console.log('📡 允許無 origin 請求（直接訪問）');
      return callback(null, true);
    }
    
    // 檢查是否在允許列表中
    if (allowedOrigins.indexOf(origin) !== -1) {
      console.log(`✅ 允許的來源: ${origin}`);
      return callback(null, true);
    }
...
const handleFHIRResponse = async (response, operation = 'FHIR operation') => {
  if (!response.ok) {
    ...
    if (response.status === 400) {
      throw new Error(`資料格式錯誤: ${errorText}`);
    } else if (response.status === 404) {
      throw new Error('資源不存在');
    } else if (response.status === 409) {
      throw new Error('資源衝突或已存在');
    } else {
      throw new Error(`FHIR API 錯誤 (${response.status}): ${errorText}`);
    }
  }
...
const setSmartCookie = (res, name, value, options = {}) => {
  const defaultOptions = {
    httpOnly: name === 'token', // 只有 token 設為 httpOnly
    maxAge: 3600000, // 1小時
    sameSite: 'lax',
    // 智慧判斷 secure
    secure: false, // 預設為 false，讓所有環境都能用
    // 重要：不設置 domain，讓瀏覽器自動處理
  };
  
  const cookieOptions = { ...defaultOptions, ...options };
  
  console.log(`🍪 設置 Cookie: ${name}, secure=${cookieOptions.secure}, httpOnly=${cookieOptions.httpOnly}`);
...
app.use((req, res, next) => {
  ...
  console.log('  Origin:', req.headers.origin || '無');
  console.log('  Host:', req.headers.host);
  console.log('  X-Forwarded-Proto:', req.headers['x-forwarded-proto']);
  console.log('  Cookies:', Object.keys(req.cookies).length > 0 ? req.cookies : '無');
...
app.get('/api/user-info', async (req, res) => {
  try {
    ...
    const name = person.name?.[0]?.text || '未命名用戶';
    ...
    res.status(401).json({ error: '獲取用戶資訊失敗' });
  }
});
...
app.post('/api/person/register', async (req, res) => {
  ...
  if (!name || !loginId || !email || !password) {
    return res.status(400).json({ error: '缺少必要欄位：姓名、登入ID、電子郵件、密碼' });
  }
  ...
  if (password.length < 4) {
    return res.status(400).json({ error: '密碼至少需要4個字符' });
  }
  
  // 驗證登入ID格式（只允許字母數字）
  if (!/^[a-zA-Z0-9_-]+$/.test(loginId)) {
    return res.status(400).json({ error: '登入ID只能包含字母、數字、破折號和下劃線' });
  }
  ...
  try {
    ...
    if (chkData.total > 0) return res.status(409).json({ error: 'Login ID 已存在' });
...
app.post('/api/login', async (req, res) => {
  ...
  if (!input || !password) return res.status(400).json({ error: '登入ID和密碼為必填' });
  ...
  try {
    ...
    }, JWT_SECRET, { expiresIn: '8h' }); // 延長有效期為 8 小時
    
    // ✅ 使用智慧 Cookie 設置
    setSmartCookie(res, 'token', token, {
      httpOnly: true,
      maxAge: 8 * 3600000 // 8小時
    });
...
app.get('/api/check-admin', async (req, res) => {
  try {
    const token = req.cookies.token;
    if (!token) {
      return res.status(401).json({ isAdmin: false, error: '未登入' });
    }
    ...
    res.status(401).json({ isAdmin: false, error: 'Token 無效' });
  }
});

// ── 5) Get Practitioner Roles ──────────────
app.get('/api/practitioner/roles', async (req, res) => {
  try {
    ...
    if (!token) {
      console.log('❌ 沒有找到 token cookie');
      return res.status(401).json({ error: 'Unauthorized - 請重新登入' });
    }
    ...
    try {
      ...
      console.log(`✅ Token 驗證成功，用戶 ID: ${decoded.id}, 管理員: ${decoded.isAdmin}`);
    } catch (jwtError) {
      console.error('❌ Token 驗證失敗:', jwtError.message);
      return res.status(401).json({ error: 'Invalid token - 請重新登入' });
    }
    ...
    for (const pid of practitionerIds) {
      // 使用 _include 和 _revinclude 來獲取完整的 Organization 資訊
      ...
      (roleData.entry || []).forEach(e => {
        ...
        for (const text of allCodeTexts) {
          if (text.includes('醫師') || text.includes('醫生') || text.includes('doctor') || text.includes('physician')) {
            roleType = 'doctor';
            break;
          } else if (text.includes('藥師') || text.includes('藥劑師') || text.includes('pharmacist')) {
            roleType = 'pharmacist';
            break;
          } else if (text.includes('護理師') || text.includes('護士') || text.includes('nurse')) {
            roleType = 'nurse';
            break;
          } else if (text.includes('助理') || text.includes('assistant')) {
            roleType = 'assistant';
            break;
          }
        }
        ...
        if (e.resource.organization && e.resource.organization.reference) {
          ...
          // 從 included 資源中獲取 Organization 名稱
    ...
    console.log(`✅ 返回 ${roles.length} 個角色`);
...
app.post('/api/practitioner/select-role', async (req, res) => {
  try {
    const token = req.cookies.token;
    if (!token) {
      console.log('❌ select-role: 沒有 token');
...
app.post('/api/request-reset', async (req, res) => {
  ...
  if (!cleanLoginId) {
    return res.status(400).json({ error: '登入ID為必填' });
  }
  ...
  try {
    ...
    if (data.total === 0) {
      console.log(`[密碼重設] 未找到登入ID: ${cleanLoginId}`);
      // 出於安全考慮，不告訴用戶登入ID是否存在
      return res.json({ 
        message: '如果該登入ID存在，重設連結已寄送到註冊信箱' 
      });
    }
    ...
    if (!emailEntry) {
      return res.status(400).json({ error: '該帳號未設定電子郵件' });
    }
    ...
    const resetToken = jwt.sign({ 
      ...
      timestamp: Date.now(),
      email: email
    }, JWT_SECRET, { expiresIn: '15m' }); // 15分鐘有效期
    ...
    if (emailResult.developmentMode) {
      responseData.developmentInfo = '開發模式：郵件未實際發送，請查看控制台獲取重設連結';
...
app.post('/api/reset-password', async (req, res) => {
  ...
  if (!cleanToken || !cleanPassword) {
    return res.status(400).json({ error: 'Token和新密碼為必填' });
  }
  ...
  try {
    ...
    if (usedTokens.has(cleanToken)) {
      return res.status(400).json({ error: '此重設連結已被使用，請申請新的重設連結' });
    }
    ...
    if (currentTime - tokenTime > 15 * 60000) { // 15分鐘
      return res.status(400).json({ error: '重設連結已過期（15分鐘），請申請新的重設連結' });
    }
    ...
    if (err.name === 'JsonWebTokenError') {
      return res.status(400).json({ error: '無效或過期的重設連結' });
    }
...
app.use((error, req, res, next) => {
  ...
  // CORS 錯誤處理
  if (error.message.includes('不允許的來源')) {
...
🌐 外部訪問：http://203.64.84.209:${PORT}
⚙️  環境：${process.env.NODE_ENV || 'development'}
🔗 FHIR 伺服器：${FHIR_BASE}
🍪 Cookie 策略：智慧設置（secure=false，兼容內外網）
🔐 管理員功能：已啟用（root 用戶）
📧 郵件服務：${process.env.SMTP_USER ? '已配置' : '未配置'}
==============================================
💡 提示：
1. Cookie secure 設為 false 以兼容 HTTP
2. 支持內外網同時訪問
3. Token 有效期：8小時
4. 重設令牌有效期：15分鐘
==============================================
